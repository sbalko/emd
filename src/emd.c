...
static void newSol();
static void russel(float *S, float *D);
static void addBasicVariable(int minI, int minJ, float *S, float *D,
...
static float L2(feature_t *f1, feature_t *f2);
...
**********************/
static float L2(feature_t *f1, feature_t *f2) {
	float diffL = f1->L - f2->L;
	float diffa = f1->a - f2->a;
	float diffb = f1->b - f2->b;
...
float emd(signature_t *Signature1, signature_t *Signature2)
{
  float (*Dist)(feature_t *, feature_t *) = &L2;
  int itr;
  float totalCost;
  float w;
  node2_t *XP;
...
extern "C" {
	float  __attribute__((used)) compute_emd(char * jsonSignature1, char * jsonSignature2) {
		printf("JSON strings received: '%s' and '%s'\n", jsonSignature1, jsonSignature2);

		signature_t signature1(jsonSignature1);
		signature_t signature2(jsonSignature2);

		printf("signatures constructed:\n");
		signature1.print();
		signature2.print();


		float distance = emd(&signature1, &signature2);
...
int main(int argc, char **argv) {
	float distance = compute_emd(argv[1], argv[2]);
